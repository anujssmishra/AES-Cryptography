...
#include<string.h>
int statearray[4][4];
int encryptedtext[32];
int key[4][4];
int t_key[44][4], f_key[44][4], temp_key[44][4];
int rounds = 0, count=0;
int flag = 0, start=0, filepointer=0;
FILE *fp;
...
void input() {
	...
	{
		fseek(fp, filepointer, 0);
		filepointer+=32;

		for(int ii=0;ii<16;ii++)
		{
			fscanf(fp, "%02x", &encryptedtext[ii]);
			if(feof(fp))
			{
				flag=1;
				break;
		...
		*/
		...
		for(int i=0;i<4;i++)
		{
			...
			{
				statearray[j][i] = encryptedtext[count];
			}
		}

		fscanf(fp, "%02x", &encryptedtext[0]);
		if(feof(fp))
		{
			flag=1;
		}
...
void addroundkey(){
	//Performing inverse add round function
	for(int i=0;i<4;i++)
	{
		for(int j=0;j<4;j++)
...
void invsubs (){
	//Performing Reverse S-Byte Substitution
	for(int i=0;i<4;i++)
	{
		for(int j=0;j<4;j++)
		{
...
void invmixcolumns () {
	...
	for(int i=0;i<4;i++)
	{
		result[0][i] = (multiply(0x0E,statearray[0][i]))^(multiply(0x0B,statearray[1][i]))^(multiply(0x0D,statearray[2][i]))^(multiply(0x09,statearray[3][i]));
		result[1][i] = (multiply(0x09,statearray[0][i]))^(multiply(0x0E,statearray[1][i]))^(multiply(0x0B,statearray[2][i]))^(multiply(0x0D,statearray[3][i]));
		result[2][i] = (multiply(0x0D,statearray[0][i]))^(multiply(0x09,statearray[1][i]))^(multiply(0x0E,statearray[2][i]))^(multiply(0x0B,statearray[3][i]));
		result[3][i] = (multiply(0x0B,statearray[0][i]))^(multiply(0x0D,statearray[1][i]))^(multiply(0x09,statearray[2][i]))^(multiply(0x0E,statearray[3][i]));
	}
...
void invshiftrows() {
	//Shifting rows towards right based on the row number
	for(int i=3;i>0;i--)
	{
		for(int k=4;k>i;k--)
		{
...
void key_ex() {
	...
	//Initialising key with the masterkey
	...
	//Initialising temporary key with masterkey
	for(int i=0;i<4;i++)
	{
		for(int j=0;j<4;j++)
		{
	...
	for(int i=4;i<44;i++)
	{
		if(i%4==0)
		{
			for(int j=0;j<4;j++)
			{
				t_key[i][j]=t_key[i-1][j];
			}
			//Shifting the 4th column words upwards
			int temp=t_key[i][0];
			for(int j=0;j<3;j++)
			{
				t_key[i][j]=t_key[i][j+1];
			}
			t_key[i][3]=temp;
			//Performing S-box substitution
			for(int j=0;j<4;j++)
			{
				t_key[i][j]= sbox[t_key[i][j]>>4][t_key[i][j]&0x0f];
			}
		...
		{
			for(int j=0;j<4;j++)
			t_key[i][j]=t_key[i-1][j]^t_key[i-4][j];
	...
	{
		int t = i*4;
		for(int c=0;c<4;c++,j++,t++)
		{
			for(int k=0;k<4;k++)
			{
				temp_key[j][k] = t_key[t][k];
			}
		}
	}
	rounds=0;
...
void output() {
	//Storing the decrypted text in Decrypted_Text.txt
	FILE *f_output;
	f_output = fopen("Decrypted_Text.txt","a");
	for(int i=0;i<4;i++)
	{
		for(int j=0;j<4;j++)
		{
			fprintf(f_output,"%c",statearray[j][i]);
		}
	}
	fclose(f_output);
}
...
int main() {
	...
	input();
	printf("The decrypted text is in file 'Decrypted_Text.txt'\n");
	return 0;
}
